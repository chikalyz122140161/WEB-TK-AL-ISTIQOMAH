...
.ekskul-table tbody tr:last-child td {
    border-bottom: none;
}
.ekskul-table tbody tr:hover td {
    background: #f9fafb;
}
.ekskul-name {
    font-weight: 600;
...
<div class="card">
    <div class="card__header">
        <h3 class="card__title">Daftar Ekstrakurikuler</h3>
        <span class="badge badge--primary">{{ count($ekstrakurikuler) }} Ekstrakurikuler</span>
    </div>
    <div class="card__body p-0">
        @if(count($ekstrakurikuler) === 0)
        <div class="empty-state" style="padding: 3rem; text-align:center;">
            <p style="color:#6b7280;margin-bottom:1rem;">Belum ada ekstrakurikuler yang ditambahkan.</p>
            <a href="{{ route('admin.ekstrakurikuler.create') }}" class="btn btn--primary">Tambah Ekstrakurikuler Pertama</a>
        </div>
        @else
        <table class="ekskul-table">
            <thead>
                <tr>
                    ...
                    <th style="width:220px;">Nama Ekstrakurikuler</th>
                    <th>Poin Penilaian</th>
                    <th style="width:160px;">Aksi</th>
                </tr>
            </thead>
            <tbody>
                @foreach($ekstrakurikuler as $i => $e)
                <tr>
                    <td class="row-no">{{ $i + 1 }}</td>
                    <td>
                        <span class="ekskul-name">{{ $e['nama'] }}</span>
                        <span class="poin-count">{{ count($e['penilaian']) }} poin</span>
                    </td>
                    <td>
                        @if(count($e['penilaian']) === 0)
                            <span class="poin-empty">Belum ada poin penilaian</span>
                        @else
                            <div class="poin-chips">
                                @foreach($e['penilaian'] as $p)
                                    <span class="poin-chip">{{ $p['nama'] }}</span>
                                @endforeach
                            </div>
                        @endif
                    </td>
                    <td>
                        <div class="btn-row">
                            <a href="{{ route('admin.ekstrakurikuler.edit', $e['id']) }}" class="btn btn--secondary btn--sm">
                            ...
                            <button type="button" class="btn btn--danger btn--sm"
                                    onclick="showDeleteModal('{{ $e['id'] }}', '{{ $e['nama'] }}')">
...
<div id="deleteModal" class="modal-overlay">
    <div class="modal-box">
        ...
        <p class="modal-message">Apakah Anda yakin ingin menghapus ekstrakurikuler <strong id="deleteEkskulName"></strong>? Tindakan ini tidak dapat dibatalkan.</p>
...
function showDeleteModal(id, name) {
    document.getElementById('deleteEkskulName').textContent = name;
