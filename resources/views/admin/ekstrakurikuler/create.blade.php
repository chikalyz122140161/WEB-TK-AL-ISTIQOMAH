...
<div class="page-header">
    <div class="page-header__left">
        <h1 class="page-header__title">Tambah Ekstrakurikuler</h1>
        <p class="page-header__subtitle">Tambah ekstrakurikuler beserta poin penilaiannya</p>
    </div>
...
<div class="card">
    <div class="card__header">
        <h3 class="card__title">Form Ekstrakurikuler</h3>
    </div>
    <div class="card__body">
        <form action="{{ route('admin.ekstrakurikuler.store') }}" method="POST" class="form" id="ekskul-form">
            @csrf

            <div class="form-group">
                <label for="nama" class="form-label required">Nama Ekstrakurikuler</label>
                <input type="text" id="nama" name="nama" class="form-input"
                    value="{{ old('nama') }}" placeholder="Contoh: Menari, Mewarnai, Bercerita" required maxlength="100">
            </div>
            ...
            <div class="poin-list" id="poin-list">
                @php $oldPoin = old('penilaian', []); @endphp
                @if(empty($oldPoin))
                    <div class="poin-empty-msg" id="poin-empty-msg">Belum ada poin penilaian. Klik tombol di bawah untuk menambah.</div>
                @else
                    @foreach($oldPoin as $idx => $p)
                    <div class="poin-row">
                        <input type="text" name="penilaian[]" class="form-input" value="{{ $p }}" placeholder="Contoh: Kerapian" maxlength="100">
...
function addPoin(value = '') {
    ...
    row.innerHTML = `
        <input type="text" name="penilaian[]" class="form-input" value="${value}" placeholder="Contoh: Kerapian" maxlength="100">
        <button type="button" class="btn-remove-poin" onclick="removePoin(this)" title="Hapus poin">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" width="16" height="16"><path fill-rule="evenodd" d="M5.47 5.47a.75.75 0 0 1 1.06 0L12 10.94l5.47-5.47a.75.75 0 1 1 1.06 1.06L13.06 12l5.47 5.47a.75.75 0 1 1-1.06 1.06L12 13.06l-5.47 5.47a.75.75 0 0 1-1.06-1.06L10.94 12 5.47 6.53a.75.75 0 0 1 0-1.06Z" clip-rule="evenodd"/></svg>
        </button>
    `;
