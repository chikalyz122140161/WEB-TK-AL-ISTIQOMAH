...
@section('content')
    ...
    <div class="card mb-4">
        ...
        <div class="card__body">
            <form method="GET" action="{{ route('admin.dapodik.index') }}"
            ...
            @if ($selectedTerm)
                <p style="margin-top:10px;font-size:13px;color:#5D4037;">
                    ...
                    <strong>Tahun Ajaran {{ $selectedTerm->academic_year }} — Semester
                        {{ ucfirst($selectedTerm->semester) }}</strong>
                </p>
            @endif
        </div>
    </div>
    ...
    <div class="info-box">
        ...
        <div class="info-box__content">
            ...
            <p>Data Pokok Pendidikan (DAPODIK) adalah sistem pendataan siswa yang dikelola oleh Kementerian Pendidikan dan
                Kebudayaan. Data ini digunakan untuk berbagai keperluan administrasi pendidikan termasuk BOS, PIP, dan
                bantuan lainnya.</p>
        </div>
    </div>
        ...
        <div class="card__body">
            <div class="filter-row">
                <div class="filter-group">
                    ...
                    <input type="text" id="search" placeholder="Nama, NISN, atau NIK..." class="form-input"
                        onkeyup="filterTable()">
                </div>
                <div class="filter-group">
    ...
    <div class="card" id="printArea">
        <div class="card__header print-header">
            <div class="print-logo">
                <img src="{{ asset('assets/img/logo.png') }}" alt="Logo TK" onerror="this.style.display='none'">
                    ...
                    <tbody>
                        @forelse($siswa as $index => $s)
                            <tr data-kelas="{{ $s['kelas'] }}"
                                data-jk="{{ $s['jenis_kelamin'] == 'Laki-laki' ? 'L' : 'P' }}">
                                <td>{{ $index + 1 }}</td>
    ...
    <script>
            ...
            if (visibleIndex === 0) {
                if (!emptyRow && tbody) {
                    ...
                    emptyRow.innerHTML = '<td colspan="' + colCount +
                        '" style="text-align:center;padding:24px;color:#5D4037;">Tidak ada siswa yang cocok dengan filter</td>';
        ...
        function exportToPDF() {
            ...
            doc.text('TK AL-ISTIQOMAH', 148.5, 15, {
                align: 'center'
            });

            doc.setFontSize(12);
            doc.setFont('helvetica', 'normal');
            doc.text('REKAP DATA SISWA UNTUK DAPODIK', 148.5, 22, {
                align: 'center'
            });

            doc.setFontSize(10);
            doc.text('Tanggal: ' + today, 148.5, 28, {
                align: 'center'
            });
            ...
            for (let i = 1; i <= pageCount; i++) {
                ...
                doc.text('Halaman ' + i + ' dari ' + pageCount, 148.5, 200, {
                    align: 'center'
                });
        ...
        function exportToCSV() {
            ...
            rows.forEach(row => {
                if (row.style.display !== 'none' && !row.closest('tbody')?.querySelector(
                        'tr[style*="display: none"]')?.contains(row)) {
