...
@section('content')
    ...
    <div class="status-banner status-banner--{{ strtolower($pendaftaran['status']) }}">
        ...
        <div class="status-banner__body">
            ...
            <div class="status-banner__date">
                Tanggal Daftar :
                <span class="status-banner__value">{{ $pendaftaran['tanggal_daftar'] }}</span>
            </div>
        </div>
    </div>
            ...
            <div class="card__header">
                <h3 class="card__title">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" width="20"
                        height="20">
                        <path
                            d="M10.5 1.875a1.125 1.125 0 0 1 2.25 0v8.219c.517.162 1.02.382 1.5.659V3.375a1.125 1.125 0 0 1 2.25 0v10.937a4.505 4.505 0 0 0-3.25 2.373 8.963 8.963 0 0 1-4-.188V3.375a1.125 1.125 0 0 1 2.25 0v7.5a.75.75 0 0 0 1.5 0V1.875ZM4.5 6.75A1.125 1.125 0 0 1 5.625 5.625h.375a.75.75 0 0 0 0-1.5h-.375A2.625 2.625 0 0 0 3 6.75v10.5A2.625 2.625 0 0 0 5.625 19.875H6a.75.75 0 0 0 0-1.5h-.375A1.125 1.125 0 0 1 4.5 17.25V6.75Z" />
                        <path
                            d="M18.375 5.625h.375a.75.75 0 0 1 0 1.5h-.375c-.621 0-1.125.504-1.125 1.125v2.344A4.467 4.467 0 0 1 18.75 10.5c.103 0 .206.003.308.009V6.75A2.625 2.625 0 0 0 16.5 4.125H16.125a.75.75 0 0 0 0 1.5H16.5c.621 0 1.125.504 1.125 1.125v.375c0 .621.504 1.125 1.125 1.125ZM18.75 12a3 3 0 1 0 0 6 3 3 0 0 0 0-6Zm4.5 3a4.5 4.5 0 1 1-9 0 4.5 4.5 0 0 1 9 0Z" />
                    </svg>
                    Data Orang Tua / Wali
                </h3>
            </div>
                ...
                @forelse($pendaftaran['dokumen'] ?? [] as $doc)
                    <div class="document-item">
                        ...
                        <div class="document-info">
                            ...
                            <span
                                class="document-type">{{ strtoupper(pathinfo($doc['file'], PATHINFO_EXTENSION)) }}</span>
                        </div>
                        <a href="{{ asset('storage/' . $doc['file']) }}" class="document-download" title="Download"
    ...
    <div id="acceptModal" class="modal-overlay">
        <div class="modal-box">
            <div class="modal-icon modal-icon--success">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
                    <path fill-rule="evenodd"
                        d="M2.25 12c0-5.385 4.365-9.75 9.75-9.75s9.75 4.365 9.75 9.75-4.365 9.75-9.75 9.75S2.25 17.385 2.25 12Zm13.36-1.814a.75.75 0 1 0-1.22-.872l-3.236 4.53L9.53 12.22a.75.75 0 0 0-1.06 1.06l2.5 2.5a.75.75 0 0 0 1.14-.1l4.14-5.794Z"
                        clip-rule="evenodd" />
                </svg>
            </div>
                ...
                <div class="form-group" style="margin-top:1rem;text-align:left;">
                    ...
                    <select name="academic_term_id" class="form-select" required>
                        ...
                        @foreach ($academicTermList as $at)
                            <option value="{{ $at->id }}">{{ $at->academic_year }} — Semester
                                {{ ucfirst($at->semester) }}</option>
                        @endforeach
                    </select>
                </div>
    ...
    <div id="rejectModal" class="modal-overlay">
        <div class="modal-box">
            ...
            <p class="modal-message">Apakah Anda yakin ingin menolak pendaftaran
                <strong>{{ $pendaftaran['nama_siswa'] }}</strong>?
            </p>
    ...
    <script>
        ...
        function closeAcceptModal() {
            ...
            document.querySelector('#acceptModal select[name="class_id"]').value = '';
            document.querySelector('#acceptModal select[name="academic_term_id"]').value = '';
        }
