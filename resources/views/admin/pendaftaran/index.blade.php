...
@section('content')
    ...
    <!-- Filter Section -->
    <div class="card mb-4 filter-card-wrapper">
        <div class="card__tabs-header">
            <div class="filter-row">
                <div class="filter-group filter-group--search">
                    <label for="search">Cari Siswa</label>
                    <div class="input-with-icon">
                        <input type="text" id="search" placeholder="Ketik nama siswa atau email..."
                            class="form-input">
                    </div>
                </div>
    ...
    <div class="tabs-nav-container">
        <div class="tabs-nav__info">
            <label>Pilih filter berdasarkan status:</label>
        </div>
                ...
                <table class="data-table">
                    ...
                    <tbody>
                        @forelse($pendaftaran as $index => $p)
                            <tr data-status="{{ strtolower($p['status']) }}" data-tanggal="{{ $p['tanggal_daftar_iso'] }}"
                                ...
                                <td>
                                    <div class="student-cell">
                                        <div
                                            class="student-cell__avatar {{ $p['jenis_kelamin'] == 'Perempuan' ? 'student-cell__avatar--female' : '' }}">
                                            {{ substr($p['nama_siswa'], 0, 1) }}
                                        </div>
                                        <div class="student-cell__info">
                                            <span class="student-cell__name">{{ $p['nama_siswa'] }}</span>
                                            <span class="student-cell__detail">{{ $p['jenis_kelamin'] }} •
                                                {{ $p['tempat_lahir'] }}, {{ $p['tanggal_lahir'] }}</span>
                                        </div>
                                    </div>
                                </td>
                                <td>
                                    <div class="parent-cell">
                                        <span class="parent-cell__name">{{ $p['nama_ayah'] }} /
                                            {{ $p['nama_ibu'] }}</span>
                                ...
                                <td>
                                    <div class="action-buttons">
                                        <a href="{{ route('admin.pendaftaran.show', ['id' => $p['id']]) }}"
                                            class="btn btn--icon btn--info" title="Lihat Detail">
                                        ...
                                        @if ($p['status'] == 'Pending')
                                            <button type="button" class="btn btn--icon btn--success"
                                                ...
                                                onclick="showAcceptModal('{{ $p['id'] }}', '{{ $p['nama_siswa'] }}')">
                                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
                                                    fill="currentColor" width="16" height="16">
                                                    <path fill-rule="evenodd"
                                            ...
                                            <button type="button" class="btn btn--icon btn--danger" title="Tolak"
                                                onclick="showRejectModal('{{ $p['id'] }}', '{{ $p['nama_siswa'] }}')">
                                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
                                                    fill="currentColor" width="16" height="16">
                                                    <path fill-rule="evenodd"
    ...
    <div id="rejectModal" class="modal-overlay">
        <div class="modal-box">
            ...
            <form id="rejectForm" method="POST">
                @csrf
                <textarea name="alasan_penolakan" class="form-textarea" rows="3" placeholder="Alasan penolakan (opsional)..."
