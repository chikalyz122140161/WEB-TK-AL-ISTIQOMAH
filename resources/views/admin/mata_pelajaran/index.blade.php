...
<div class="card">
    <div class="card__header">
        <h3 class="card__title">Daftar Mata Pelajaran</h3>
        <span class="badge badge--primary">{{ count($mataPelajaran) }} Mata Pelajaran</span>
    </div>
    <div class="card__body p-0">
        @if(count($mataPelajaran) === 0)
        <div class="empty-state" style="padding: 3rem; text-align:center;">
            <p style="color:#6b7280;margin-bottom:1rem;">Belum ada mata pelajaran yang ditambahkan.</p>
            <a href="{{ route('admin.mata_pelajaran.create') }}" class="btn btn--primary">Tambah Mata Pelajaran Pertama</a>
        </div>
        @else
        <table class="mapel-table">
            ...
            <tbody>
                @foreach($mataPelajaran as $i => $m)
                <tr>
                    <td class="row-no">{{ $i + 1 }}</td>
                    <td><span class="mapel-name">{{ $m['nama'] }}</span></td>
                    <td>
                        <div class="btn-row">
                            <a href="{{ route('admin.mata_pelajaran.edit', $m['id']) }}" class="btn btn--secondary btn--sm">
                            ...
                            <button type="button" class="btn btn--danger btn--sm"
                                    onclick="showDeleteModal('{{ $m['id'] }}', '{{ $m['nama'] }}')">
...
<div id="deleteModal" class="modal-overlay">
    <div class="modal-box">
        ...
        <p class="modal-message">Apakah Anda yakin ingin menghapus mata pelajaran <strong id="deleteMapelName"></strong>? Tindakan ini tidak dapat dibatalkan.</p>
...
function showDeleteModal(id, name) {
    ...
    document.getElementById('deleteForm').action = "{{ url('admin/mata-pelajaran') }}/" + id;
