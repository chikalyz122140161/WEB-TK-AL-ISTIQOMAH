...
<style>
    ...
    .cl__top { padding: 16px; border-bottom: 1px solid #f3f4f6; background: #fff; }
    .cl__title {
    ...
    .btn-new-chat {
        font-size: 11px; font-weight: 600; color: #4CAF82;
        background: #ecfdf5; border: 1px solid #bbf7d0; border-radius: 6px;
        padding: 3px 8px; cursor: pointer; text-decoration: none; transition: background .12s;
    }
    ...
    .ci {
        ...
        cursor: pointer; text-decoration: none; transition: background .12s; position: relative;
    }
    .ci:hover { background: #f0fdf4; }
    .ci.active { background: #ecfdf5; border-left: 3px solid #4CAF82; }
    .ci__avatar {
        width: 42px; height: 42px; border-radius: 50%;
        background: linear-gradient(135deg, #4CAF82, #2E8B60); color: #fff;
        display: flex; align-items: center; justify-content: center;
        font-size: 15px; font-weight: 700; flex-shrink: 0;
    }
    .ci.active .ci__avatar { background: linear-gradient(135deg, #3D9B72, #2E8B60); }
    .ci__body { flex: 1; min-width: 0; }
    .ci__name { font-size: 13px; font-weight: 700; color: #3E2723; margin-bottom: 1px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    .ci__sub { font-size: 11px; color: #6b7280; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    .ci__preview { font-size: 11px; color: #9ca3af; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; margin-top: 2px; }
    .ci__time { font-size: 10px; color: #9ca3af; flex-shrink: 0; align-self: flex-start; margin-top: 2px; }

    .cw { flex: 1; display: flex; flex-direction: column; overflow: hidden; }
    .cw__head {
        padding: 12px 20px; border-bottom: 1px solid #f3f4f6; background: #fff;
    ...
    .cw__body {
        ...
        background: #f8faf9; display: flex; flex-direction: column; gap: 4px;
    }
    .cw__empty {
        flex: 1; display: flex; flex-direction: column;
        align-items: center; justify-content: center; color: #9ca3af; gap: 10px;
    }
    .cw__empty svg { width: 48px; height: 48px; opacity: .35; }
    .cw__empty p { font-size: 13px; }

    .msg-date { text-align: center; margin: 10px 0; }
    .msg-date span { display: inline-block; padding: 3px 12px; background: #e5e7eb; border-radius: 99px; font-size: 11px; color: #6b7280; }
    ...
    .br__av {
        ...
        background: linear-gradient(135deg, #FFF176, #e6db00); color: #3E2723;
        display: flex; align-items: center; justify-content: center;
        font-size: 11px; font-weight: 700; flex-shrink: 0;
    }
    .br--out .br__av { background: linear-gradient(135deg, #4CAF82, #2E8B60); color: #fff; }

    .bubble { max-width: 62%; padding: 10px 14px; font-size: 13px; line-height: 1.55; }
    .bubble--in {
    ...
    .bubble--out {
        background: linear-gradient(135deg, #4CAF82, #2E8B60); color: #fff;
        border-radius: 14px 14px 3px 14px; box-shadow: 0 2px 6px rgba(61,155,114,0.25);
    }
    .bubble__time { display: block; font-size: 10px; margin-top: 4px; color: rgba(0,0,0,0.3); text-align: right; }
    ...
    .cw__input {
        flex: 1; height: 42px; padding: 0 16px; font-size: 13px; color: #3E2723;
        background: #f3f4f6; border: 1.5px solid transparent; border-radius: 99px;
        outline: none; font-family: inherit; transition: all .15s;
    }
    .cw__input:focus { border-color: #4CAF82; background: #fff; box-shadow: 0 0 0 3px rgba(76,175,130,0.12); }
    ...
    .cw__send {
        ...
        display: flex; align-items: center; justify-content: center;
        flex-shrink: 0; transition: all .15s; box-shadow: 0 2px 6px rgba(61,155,114,0.3);
    }
    ...
    #newChatModal .modal-box {
        width: 360px; max-height: 480px; max-width: 100%;
...
<div class="chat-wrap">
    ...
    <div class="cl">
        <div class="cl__top">
            ...
            <input type="text" class="cl__search" id="clSearch" placeholder="Cari guru...">
        </div>
        <div class="cl__body" id="clBody">
            @forelse ($kontak as $k)
            <a href="{{ route('orangtua.chat', ['room' => $k['room_id']]) }}"
    ...
    <div class="cw">
        ...
        <div class="cw__body" id="chatBody">
            ...
            <div class="br br--{{ $p['is_mine'] ? 'out' : 'in' }}">
                ...
                @if($p['is_mine'])
                <div class="br__av">{{ strtoupper(mb_substr(auth()->user()->name ?? 'I', 0, 1)) }}</div>
                @endif
            </div>
            @endforeach
            @endif
...
<div class="modal-overlay" id="newChatModal">
    <div class="modal-box">
        ...
        <div style="padding:12px 16px;border-bottom:1px solid #f3f4f6;">
            <input type="text" id="modalSearch" placeholder="Cari guru / admin..."
        ...
        <div class="modal-body" id="modalUserList">
            ...
            <form method="POST" action="{{ route('orangtua.open_chat_room') }}"
                  class="ot-form" data-name="{{ strtolower($u['nama']) }}">
                @csrf
                ...
                <button type="submit" class="ot-item" style="width:100%;text-align:left;background:none;border:none;font-family:inherit;">
                    <div class="ot-av">{{ $u['initial'] }}</div>
                    <div style="flex:1;min-width:0;">
                        <div style="font-size:13px;font-weight:600;color:#3E2723;">{{ $u['nama'] }}</div>
                        <div style="font-size:11px;color:#6b7280;margin-top:1px;">{{ $u['role'] }}</div>
                    </div>
            ...
            @else
            <div class="modal-empty">Semua guru & admin sudah memiliki percakapan aktif</div>
            @endif
        </div>
    </div>
</div>
...
<script>
    ...
    ['btnNewChat', 'btnNewChat2'].forEach(id => {
