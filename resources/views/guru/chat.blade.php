...
@section('content')
    ...
    <div class="chat-wrap {{ $aktifId ? 'show-chat' : '' }}">
        ...
        <div class="cw">
            @if ($aktif)
                {{-- Header --}}
                <div class="cw__head">
                    <button type="button" class="cw__back-btn" id="btnBackToContacts">
                ...
                <div class="cw__body" id="chatBody">
                    @if (count($pesan) === 0)
                        <div style="margin:auto;text-align:center;color:#4CAF82;font-size:12px;">
                    ...
                    @else
                        ...
                        @foreach ($pesan as $p)
                            <div class="br br--{{ $p['is_mine'] ? 'out' : 'in' }}">
                                ...
                                @if ($p['is_mine'])
                                    <div class="br__av">{{ strtoupper(mb_substr(auth()->user()->name ?? 'G', 0, 1)) }}
                                    </div>
                                @endif
                            </div>
                        @endforeach
            ...
            @else
                ...
                <div class="cw__empty">
                    ...
                    <p style="color: #4CAF82;">Pilih percakapan atau mulai chat baru</p>
            ...
            <div class="modal-body" id="modalUserList">
                @if (count($orangtua) > 0)
                    @foreach ($orangtua as $ot)
                        <form method="POST" action="{{ route('guru.open_chat_room') }}" class="ot-form"
                            data-name="{{ strtolower($ot['nama']) }}">
                            @csrf
...
@push('scripts')
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            const btnBack = document.getElementById('btnBackToContacts');
            ...
            if (btnBack && chatWrap) {
                btnBack.addEventListener('click', function(e) {
                    ...
                    setTimeout(() => {
                        window.location.href = "{{ route('guru.chat', ['room' => '']) }}".split(
                            '?')[0];
                    }, 200);
                });
            }
