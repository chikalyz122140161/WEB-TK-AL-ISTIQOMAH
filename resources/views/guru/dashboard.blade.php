...
@section('content')
    ...
    @push('styles')
        <style>
            ...
            .dbs-green__top>div:last-child .dbs-label {
                justify-content: flex-end;
            }
            ...
            .dbs-yellow-row {
                ...
                flex-direction: row;
                align-items: center;
                border: 1px solid rgba(62, 39, 35, .05);
            }
    ...
    <div class="db-stat-row">
        ...
        <div class="dbs-green">
            ...
            <div class="dbs-green__bar-label">{{ $hadirS }} dari {{ $totalS }} hadir ({{ $pct }}%)
            </div>
            {{-- Selector class term --}}
            <form method="GET" action="{{ route('guru.dashboard') }}" style="margin-top:12px;">
                <select name="class_term_id" onchange="this.form.submit()"
                    ...
                    @foreach ($classTerms ?? [] as $ct)
                        <option value="{{ $ct->id }}" {{ $ct->id == ($selectedClassTermId ?? '') ? 'selected' : '' }}
                            style="color:#333;">
